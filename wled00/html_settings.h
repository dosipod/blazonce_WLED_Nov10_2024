...
var d=document,laprev=55;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#led-settings")}function B(){window.open("/settings","_self")}function S(){GetV(),setABL(),sTY()}function enABL(){var e=d.getElementById("able").checked;d.Sf.LA.value=e?laprev:0,d.getElementById("abl").style.display=e?"inline":"none",d.getElementById("psu2").style.display=e?"inline":"none",d.Sf.LA.value>0&&setABL()}function enLA(){var e=d.Sf.LAsel.value;d.Sf.LA.value=e,d.getElementById("LAdis").style.display=50==e?"inline":"none",UI()}function setABL(){switch(d.getElementById("able").checked=!0,d.Sf.LAsel.value=50,parseInt(d.Sf.LA.value)){case 0:d.getElementById("able").checked=!1,enABL();break;case 30:d.Sf.LAsel.value=30;break;case 35:d.Sf.LAsel.value=35;break;case 55:d.Sf.LAsel.value=55;break;case 255:d.Sf.LAsel.value=255;break;default:d.getElementById("LAdis").style.display="inline"}UI()}function UI(){var e=d.querySelectorAll(".wc"),n=e.length;for(i=0;i<n;i++)e[i].style.display=d.getElementById("rgbw").checked?"inline":"none";d.getElementById("ampwarning").style.display=d.Sf.MA.value>7200?"inline":"none",255==d.Sf.LA.value?laprev=12:d.Sf.LA.value>0&&(laprev=d.Sf.LA.value);var l=d.getElementsByTagName("input"),t=0;for(i=0;i<l.length;i++){var a=l[i].name.substring(0,2);if("LC"==a&&(t+=parseInt(l[i].value)),"LP"==a){var s=d.getElementsByName("LC"+l[i].name.substring(2))[0];3==l[i].value?s.max=500:s.max=1500}if(("LP"==a||"LK"==a||"RL"==a||"BT"==a||"IR"==a||"AX"==a)&&""!=l[i].value&&"-1"!=l[i].value)for(j=0;j<l.length;j++)if(i!=j){var r=l[j].name.substring(0,2);"LP"!=r&&"LK"!=r&&"RL"!=r&&"BT"!=r&&"IR"!=r&&"AX"!=r||""!=l[j].value&&l[i].value==l[j].value&&(alert("Pin clash!"),l[j].value="")}}d.getElementById("ledwarning").style.display=t>1e3?"inline":"none";var u=Math.ceil((100+t*laprev)/500)/2;u=u>5?Math.ceil(u):u;var m="",p=30==d.Sf.LAsel.value,v=255==d.Sf.LAsel.value;u<1.02&&!p&&!v?m="ESP 5V pin with 1A USB supply":(m+=p?"12V ":v?"WS2815 12V ":"5V ",m+=u,m+="A supply connected to LEDs");var y=Math.ceil((100+d.Sf.LC.value*laprev)/1500)/2,c="(for most effects, ~";c+=y=y>5?Math.ceil(y):y,c+="A is enough)<br>",d.getElementById("psu").innerHTML=m,d.getElementById("psu2").innerHTML=v?"":c}function sTY(){var e=d.getElementsByName("LTsel")[0],n=d.getElementsByName("iLK");for(d.getElementById("rgbw").checked=30==parseInt(e.value)||54==parseInt(e.value),i=0;i<n.length;i++){var l=d.getElementsByName("LK"+(i>0?i:""))[0];e.value>49&&54!=e.value?(n[i].style="display:inline;",l.required=!0):(n[i].style="display:none;",l.required=!1,l.value="")}UI()}function addLEDs(e){if(e>1)return d.maxST=e,void(d.getElementById("+").style="display:inline;");var n=d.getElementsByName("iST"),l=n.length;if(!(1==e&&n.length>=d.maxST||-1==e&&n.length<=1)){var t=d.getElementById("mLC");if(1==e){var a=d.createElement("div");a.setAttribute("name","iST"),a.appendChild(d.createTextNode("Strip "+(l+1)+" pin: "));var i=d.createElement("input");i.type="number",i.value="",i.name="LP"+l,i.min=0,i.max=40,i.required=!0,i.onchange=function(){UI()},a.appendChild(i);var s=d.createElement("div");s.setAttribute("name","iLK"),d.getElementsByName("LTsel")[0].value<50&&(s.style="display:none;"),s.appendChild(d.createTextNode(" clock:")),(i=d.createElement("input")).type="number",i.value="",i.name="LK"+l,i.min=0,i.max=40,i.onchange=function(){UI()},d.getElementsByName("LTsel")[0].value>49&&(i.required=!0),s.appendChild(i),a.appendChild(s),a.appendChild(d.createTextNode(" count: ")),(i=d.createElement("input")).type="number",i.value="",i.name="LC"+l,i.min=0,i.max=1e3,i.required=!0,i.oninput=function(){UI()},a.appendChild(i),a.appendChild(d.createElement("br")),t.appendChild(a),l++}-1==e&&n[--l].remove();var r=d.getElementById("+"),u=d.getElementById("-");l<d.maxST?r.style="display:inline":r.style="display:none",u.style=l>1?"display:inline":"display:none",UI()}}function GetV() {var d=document;
...
<button type="submit">Save</button><hr><h2>LED &amp; HW setup</h2>LED type: 
<select name="LTsel" onchange="sTY()"><option value="22" selected="selected">
WS281x (5V/12V)</option><option value="30">SK6812 (RGBW,5V)</option><option 
value="50">WS2801 (5V)</option><option value="51">SK9822(APA102) (5V)</option>
<option value="52">LPD8806 (5V)</option><option value="53">P9813</option><option
 value="54">TM1814 (RGBW)</option></select><button type="button" id="+" 
...
<div name="iST">Strip 1 pin: <input name="LP" type="number" min="0" max="40" 
required onchange="UI()"><div name="iLK">clock: <input name="LK" type="number" 
min="0" max="40" onchange="UI()"></div>count: <input name="LC" type="number" 
min="1" max="1500" oninput="UI()" required></div></div><hr>Relay pin: <input 
type="number" min="-1" max="40" name="RL" onchange="UI()"> Active high? <input 
type="checkbox" name="RM"><br>Button pin: <input type="number" min="-1" 
max="40" name="BT" onchange="UI()"><br>IR pin: <input type="number" min="-1" 
max="40" name="IR" onchange="UI()"><br>AUX pin: <input type="number" min="-1" 
max="40" name="AX" onchange="UI()"><hr><div id="ledwarning" 
