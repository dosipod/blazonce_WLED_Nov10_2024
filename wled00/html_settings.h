...
var d=document,laprev=55;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#led-settings")}function B(){window.open("/settings","_self")}function S(){GetV(),setABL()}function enABL(){var e=d.getElementById("able").checked;d.Sf.LA.value=e?laprev:0,d.getElementById("abl").style.display=e?"inline":"none",d.getElementById("psu2").style.display=e?"inline":"none",d.Sf.LA.value>0&&setABL()}function enLA(){var e=d.Sf.LAsel.value;d.Sf.LA.value=e,d.getElementById("LAdis").style.display=50==e?"inline":"none",UI()}function setABL(){switch(d.getElementById("able").checked=!0,d.Sf.LAsel.value=50,parseInt(d.Sf.LA.value)){case 0:d.getElementById("able").checked=!1,enABL();break;case 30:d.Sf.LAsel.value=30;break;case 35:d.Sf.LAsel.value=35;break;case 55:d.Sf.LAsel.value=55;break;case 255:d.Sf.LAsel.value=255;break;default:d.getElementById("LAdis").style.display="inline"}UI()}function UI(){var e=d.querySelectorAll(".wc"),n=e.length;for(i=0;i<n;i++)e[i].style.display=d.getElementById("rgbw").checked?"inline":"none";d.getElementById("ledwarning").style.display=d.Sf.LC.value>1e3?"inline":"none",d.getElementById("ampwarning").style.display=d.Sf.MA.value>7200?"inline":"none",255==d.Sf.LA.value?laprev=12:d.Sf.LA.value>0&&(laprev=d.Sf.LA.value);var t=d.getElementsByTagName("input"),l=0;for(i=0;i<t.length;i++)"LC"==t[i].name.substring(0,2)&&(l+=parseInt(t[i].value));var a=Math.ceil((100+l*laprev)/500)/2;a=a>5?Math.ceil(a):a;var s="",p=30==d.Sf.LAsel.value,u=255==d.Sf.LAsel.value;a<1.02&&!p&&!u?s="ESP 5V pin with 1A USB supply":(s+=p?"12V ":u?"WS2815 12V ":"5V ",s+=a,s+="A supply connected to LEDs");var r=Math.ceil((100+d.Sf.LC.value*laprev)/1500)/2,c="(for most effects, ~";c+=r=r>5?Math.ceil(r):r,c+="A is enough)<br>",d.getElementById("psu").innerHTML=s,d.getElementById("psu2").innerHTML=u?"":c}function addLEDs(e,n){if(!(e>n)){var t=d.getElementById("mLC");if(e>0){var l=d.getElementById("LB"+(e-1));l&&(l.style.display="none"),t.appendChild(d.createElement("br")),t.appendChild(d.createTextNode("Strip "+(e+1)+" pin D:"));var a=d.createElement("input");a.type="number",a.value="",a.name="LP"+e,a.min=0,a.max=20,t.appendChild(a),t.appendChild(d.createTextNode(" C:")),(a=d.createElement("input")).type="number",a.value="",a.name="LK"+e,a.min=0,a.max=20,t.appendChild(a),t.appendChild(d.createTextNode(" & count: ")),(a=d.createElement("input")).type="number",a.value="",a.name="LC"+e,a.min=0,a.max=500,a.oninput=function(){UI()},t.appendChild(a)}e<n-1&&((a=d.createElement("button")).type="button",a.id="LB"+e,a.onclick=function(){addLEDs(e+1,n)},a.innerText="+",t.appendChild(a))}}function GetV() {var d=document;
...
<button type="submit">Save</button><hr><h2>LED setup</h2><div id="mLC">
LED pin D:<input name="LP" type="number" min="0" max="20"> C:<input name="LK" 
type="number" min="0" max="20"> &amp; count: <input name="LC" type="number" 
min="1" max="1500" oninput="UI()" required></div><br><div id="ledwarning" 
...
type="number" min="-1" max="40" name="RL"> Active low? <input type="checkbox" 
